...
class MeasurementFrame(ctk.CTkFrame):
    ...
    def buildFrame(self):
        ...
        self.btn_start_meas = ctk.CTkButton(btnFrm, text='START', width=20, font=('Helvetica bold', 24), command=self.start_stop)
...
class ConfigFrame(ctk.CTkFrame):
    ...
    def buildFrame(self):
        ...
        cfgFrm = ctk.CTkFrame(self)
        cfgFrm.place(relx=0.0, rely=0, relwidth=1, relheight=1)

        # create and place column headers
        ctk.CTkLabel(cfgFrm, text='Conversion Time', font=('Helvetica', 16)).grid(row=0, column=1, padx=20, pady=5, sticky='we')
        ctk.CTkLabel(cfgFrm, text='Range', font=('Helvetica', 16)).grid(row=0, column=2, padx=20, pady=5, sticky='we')

        # create and place column 0 labels: titles
        ctk.CTkLabel(cfgFrm, text='Vbus', font=('Helvetica', 14)).grid(row=1, column=0, padx=5, pady=10, sticky='e')
        ctk.CTkLabel(cfgFrm, text='Vshunt', font=('Helvetica', 14)).grid(row=2, column=0, padx=5, pady=10, sticky='e')
        ctk.CTkLabel(cfgFrm, text='Mode', font=('Helvetica', 14)).grid(row=3, column=0, padx=5, pady=10, sticky='e')
        ctk.CTkLabel(cfgFrm, text='Rshunt Value (Ohm)', font=('Helvetica', 14)).grid(row=4, column=0, padx=5, pady=10, sticky='e')
        ctk.CTkLabel(cfgFrm, text='Configuration Reg', font=('Helvetica', 14)).grid(row=5, column=0, padx=5, pady=10, sticky='e')
        ctk.CTkLabel(cfgFrm, text='Calibration Reg', font=('Helvetica', 14)).grid(row=6, column=0, padx=5, pady=10, sticky='e')

        # create combo boxes: selecting DPM paramters for Configuration and Calibration Registers
        self.bus_conv_time_cb = ctk.CTkComboBox(cfgFrm, values=self.adc_conv_time, font=('Helvetica', 14), command=self.get_combo_selection)
        self.bus_vrange_cb = ctk.CTkComboBox(cfgFrm, values=self.bus_vrange, font=('Helvetica', 14), command=self.get_combo_selection)
        self.sh_conv_time_cb = ctk.CTkComboBox(cfgFrm, values=self.adc_conv_time, font=('Helvetica', 14), command=self.get_combo_selection)
        self.sh_vrange_cb = ctk.CTkComboBox(cfgFrm, values=self.sh_vrange, font=('Helvetica', 14), command=self.get_combo_selection)
        self.conv_mode_cb = ctk.CTkComboBox(cfgFrm, values=self.adc_conv_mode, font=('Helvetica', 14), command=self.get_combo_selection)
        self.ent_rshunt = ctk.CTkLabel(cfgFrm, text=dp.rshunt, anchor='center', font=('Helvetica', 16), takefocus=True)
        self.cfg_reg_lbl = ctk.CTkLabel(cfgFrm, text='', anchor='center', font=('Helvetica', 16))
        self.cal_reg_lbl = ctk.CTkLabel(cfgFrm, text='calib', anchor='center', font=('Helvetica', 16))
    ...
    def entry_lbl_focus_in(self, event):
        ...
        self.ent_rshunt.configure(fg_color='#888')
