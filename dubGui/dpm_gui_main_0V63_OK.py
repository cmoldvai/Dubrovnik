...
class App(tk.Tk):
    ...
    def on_closing(self):
        comFrm.saveConfig()
        if messagebox.askokcancel("Quit (saving params)", "Do you want to quit?"):
            self.destroy()
...
class ConfigFrame(ttk.Frame):
    ...
    def buildFrame(self):
        ...
        self.ent_rshunt = ttk.Entry(self, textvariable=self.rshunt_var)
        ...
        self.rshunt_var.set('Hello Rshunt Value')

        ttk.Label(self, text='Config Reg').grid(
            row=5, column=0, padx=5, pady=5, sticky='e')
        self.cfg_reg_lbl = ttk.Label(self, text='', anchor='w')
        ...
        #     self, text='Apply_Save???', width=10, command=self.apply_dpm_config)
        # self.btn_apply.grid(
        #     row=6, column=1, padx=10, pady=2, sticky='we')
        ...
        # When <Return> pressed or box loses focus validate the value in the box
        self.ent_rshunt.bind('<Return>', self.validate_rshunt_val)
        self.ent_rshunt.bind('<FocusOut>', self.validate_rshunt_val)
    ...
    def get_combo_selection(self, event):
        #TODO Should thid update the dp values and/or ConfigReg Label? 
        # Or let the 'Apply' button do that?
    ...
    def validate_rshunt_val(self, event):
        try:
            inp = float(self.ent_rshunt.get())
            dp.rshunt = inp
            print(f'Value entered: {inp}')
            self.ent_rshunt.tk_focusNext().focus()
            self.apply_dpm_config()
        except ValueError:
            showerror('Invalid Number', 'Must be a decimal number. Try Again')
            self.ent_rshunt.focus_set()
            print('Invalid entry')
    ...
    def apply_dpm_config(self):
        ...
        # comFrm.saveConfig()
        # self.set_config_reg_label()
        # The lines below were moved into the set_config_reg_label() function
        # b = f'0 {dp.brng:02b} {dp.pg:02b} {dp.badc:04b} {dp.sadc:04b} {dp.mode:03b}'
        # w = (dp.brng << 13) | (dp.pg << 11) | (
        #     dp.badc << 7) | (dp.sadc << 3) | dp.mode
        # msg = b + f' = {w:04X}h'
