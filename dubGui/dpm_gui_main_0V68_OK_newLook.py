...
class SerComFrame(ttk.Frame):
    ...
    def buildFrame(self):
        ...
        chk = tk.Checkbutton(comFrm, text='Auto connect', variable=self.chk_var, font=('Calibri', 11))
        chk.grid(row=0, column=0, pady=20)

        self.combo = ttk.Combobox(
            comFrm, width=10, values=self.portList, postcommand=self.updtPortList, font=('Calibri', 11))
        self.combo.grid(row=0, column=1, padx=10, pady=20)

        self.btn2 = tk.Button(comFrm, text='Connect', width=10,
                               command=self.connectPort, font=('Calibri', 11))
...
class MeasurementFrame(ttk.Frame):
    ...
    def buildFrame(self):
        ...
        ttk.Label(measFrm, text='Vshunt').grid(
        ...
        ttk.Label(measFrm, text='Vbus').grid(
        ...
        ttk.Label(measFrm, text='R load').grid(
            row=4, column=0, padx=5, pady=5, sticky='e')
        self.rload_lbl = ttk.Label(measFrm, text="0", relief='flat', width=20,
                               anchor='e', foreground='gray', background='#f8f8f8', font=('Calibri', 12))
        ...
        self.btn_start_meas = tk.Button(
            # btnFrm, text='START', width=10, command=self.start_stop)  # removed font=('Calibri bold', 18),
            btnFrm, text='START', width=10, font=('Calibri bold', 18), command=self.start_stop)  # removed font=('Calibri bold', 18),
...
class ConfigFrame(ttk.Frame):
    ...
    def buildFrame(self):
        ...
        configFrm.place(relx=0.0, rely=0.1, relwidth=.8, relheight=1)
        ...
        ttk.Label(configFrm, text='Conversion Time',font=('Calibri bold', 11)).grid(
            row=0, column=1, padx=20, pady=5, sticky='we')

        ttk.Label(configFrm, text='Range',font=('Calibri bold', 11)).grid(
            row=0, column=2, padx=20, pady=5, sticky='we')

        ttk.Label(configFrm, text='Vbus',font=('Calibri', 10)).grid(
            row=1, column=0, padx=5, pady=10, sticky='e')

        ttk.Label(configFrm, text='Vshunt',font=('Calibri', 10)).grid(
            row=2, column=0, padx=5, pady=10, sticky='e')

        ttk.Label(configFrm, text='Mode',font=('Calibri', 10)).grid(
            row=3, column=0, padx=5, pady=10, sticky='e')

        self.bus_conv_time_cb = ttk.Combobox(
            configFrm, width=12, values=self.adc_conv_time,font=('Calibri', 11))
        ...
        self.bus_vrange_cb = ttk.Combobox(
            configFrm, width=12, values=self.bus_vrange,font=('Calibri', 11))
        ...
        self.sh_vrange_cb = ttk.Combobox(
            configFrm, width=12, values=self.sh_vrange,font=('Calibri', 11))
        ...
        self.conv_mode_cb = ttk.Combobox(
            configFrm, width=8, values=self.adc_conv_mode,font=('Calibri', 11))
        self.conv_mode_cb.grid(
            row=3, column=1, columnspan=2, padx=20, pady=10, sticky='we')
        self.conv_mode_cb.set(self.adc_conv_mode[7])

        ttk.Label(configFrm, text='Rshunt Value (Ohm)',font=('Calibri', 10)).grid(
            row=4, column=0, padx=5, pady=10, sticky='e')
        self.ent_rshunt = ttk.Label(configFrm, text=dp.rshunt, relief='sunken', background='#fff', anchor='e', takefocus=True, font=('Calibri', 11))
