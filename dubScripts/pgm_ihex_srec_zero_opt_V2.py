...
def write_buf_write(inp_arr, dsize, prev_zero):
    ...
    if zero_array:
        if not prev_zero:
            comm.send("pattern 0 0")
        return True

    # we get here only if the current array is not all zeroes
    offst = 0
    while offst < dsize:
        if dsize - offst >= 16:
            end_offst = offst + 16
...
def data_program_and_verify(data_array, start_addr, do_verify):
    ...
    rv = '0'
    # pz is a boolean indicating whether the previous array was all zero
    pz = False
    ...
    while addr < end_addr:
        ...
        pz = write_buf_write(data_array[idx:idx+prog_size], prog_size, pz)
...
# print('set voltage 0')
# comm.send('volt 0; delay 3000000')
# print('set voltage 3.3V')
# comm.send('volt 3.3; delay 1000000')
# print('setpart')
